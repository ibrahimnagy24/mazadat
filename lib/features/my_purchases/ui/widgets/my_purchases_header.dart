...
class _MyPurchasesHeaderState extends State<MyPurchasesHeader> {
  ...
  void _onSearchChanged(String query, MyPurchasesCubit cubit) {
    _debounceTimer?.cancel();
    _debounceTimer = Timer(const Duration(milliseconds: 750), () {
      if (query.isEmpty) {
        cubit.clearSearch();
      } else {
        cubit.searchShipments(query);
      }
    });
  }
                ...
                children: [
                  Expanded(
                    child: Column(
                      children: [
                        ...
                        if (state is MyPurchasesSearchLoading)
                          Container(
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: SvgPicture.asset(
                        cubit.displayType == MyPurchasesDisplayTypes.list
                            ? AppSvg.grid
                            : AppSvg.list,
                        width: 20,
                        height: 20,

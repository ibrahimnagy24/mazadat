      ...
      child: BlocBuilder<BundleDetailsCubit, BundleDetailsState>(
        builder: (context, state) {
          ...
          if (state is BundleDetailsSuccess ||
              state is BundleDetailsBackgroundRefresh) {
            final bundleDetails = state is BundleDetailsSuccess
                ? state.bundleDetails
                : (state as BundleDetailsBackgroundRefresh).bundleDetails;

            return ListAnimator(
              padding: EdgeInsets.symmetric(horizontal: 24.w),
              data: List.generate(
                bundleDetails.auction?.length ?? 0,
                (i) => Padding(
                  padding: EdgeInsets.symmetric(vertical: 8.h),
                  child: ListAuctionCard(auction: bundleDetails.auction![i]),
                ),
              ),
            );
          }

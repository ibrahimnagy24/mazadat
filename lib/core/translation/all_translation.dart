...
class GlobalTranslations {
  ...
  Future<void> setNewLanguage(
    ...
    // String? language = newLanguage ?? await getPreferredLanguage();
    if (saveInPrefs) {
      await setPreferredLanguage(language);
      mainAppBloc.updateLang(language);
      // log('SaveInPrefs====>${language}');
    }
    // language = await getPreferredLanguage();
    // log('getPreferredLanguage===>${language}');
  ...
  Future<String> _getApplicationSavedInformation(String name) async {
    return await SharedHelper.box!.get(_storageKey + name) ?? 'ar';
  }
